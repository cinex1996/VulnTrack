...
from .models import Vulnerability, History
...
def index(request):
    ...
    critical_vulnerabilities = vulnerabilities.filter(
        severity="critical"
    ).count()
    fixed_vulnerabilities = vulnerabilities.filter(
        status="fixed"
    ).count()
    ...
    if request.method == "POST":
        if "comment_submit" in request.POST:
            ...
            if comment_form.is_valid():
                ...
                if request.user != vulnerability.reporter:
                    Notification.objects.create(
                        ...
                        url=(
                            f"/vulnerabilities/detail/{vulnerability.id}/"
                        ),
                    )
            ...
            status_form = StatusUpdateForm(
                request.POST, instance=vulnerability
            )
...
def update_vulnerability(request, id):
    ...
    return render(
        request,
        "vulnerabilities/edit.html",
        {"form": form, "vulnerability": vulnerability},
    )
